...
void Player::setMosaic(const string mosaic[6][6]) {
	for (int i = 0; i < 6; i++)
		this->mosaic[i][i] = mosaic[i][i];
}

void Player::setMosaic2(const string mosaic[6][6]) {
	for (int i = 0; i < 6; i++)
		this->mosaic[i][i] = mosaic[i][i];
}

void Player::setMosaicSquare(const string mosaic_square[6][6]) {
	for (int i = 1; i <= 5; i++)
		this->mosaic_square[i][i] = mosaic_square[i][i];
}

void Player::setMosaicSquare2(const string mosaic_square[6][6]) {
	for (int i = 1; i <= 5; i++)
		this->mosaic_square[i][i] = mosaic_square[i][i];
}

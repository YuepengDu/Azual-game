...
void Game::processFactory_Mosaic(Player* player, string factory[6][6], string mosaic[6][6], int factory_line, char title_code, int storage_row) {
	string lineFactory = "";
	string lineMosaic = "";
	string lineBroken = "";
...
void Game::processFactory_Mosaic2(Player* player, string factory[6][6], string mosaic[6][6], int factory_line, char title_code, int storage_row) {
	string lineFactory = "";
	string lineMosaic = "";
	string lineBroken = "";
...
void Game::checkSquare(Player* player, string lineSquare, int position, char color, string mosaic_square[6][6], int &score, int lineS) {
	int count = 0;
...
void Game::processRectagle_Square(Player* player, string mosaic[6][6], string mosaic_square[6][6], int &score) {
	string lineSquare = "";
	string lineMosaic = "";
	string mosaicClone[6][6];
			...
			if (i == j) {
				for (int z = 0; z < lineMosaic.length(); z++) {
					...
					if (lineMosaic[z] == 'B') {
						count++;
						if (count == j) {
							checkSquare(player, lineSquare, i, lineMosaic[z], mosaic_square, score, i);
						...
						if (lineMosaic[z] == 'Y') {
							...
							if (count == j) {
								if ((i + 1) > 5) {
									checkSquare(player, lineSquare, i - 4, lineMosaic[z], mosaic_square, score, i);
								}
								else
									checkSquare(player, lineSquare, i + 1, lineMosaic[z], mosaic_square, score, i);
								mosaic_square[i][i] = player->getLineSquare();
								mosaic[j][j] = mosaicClone[j][j];
								break;
							}
							...
							if (lineMosaic[z] == 'R') {
								...
								if (count == j) {
									if ((i + 2) > 5) {
										checkSquare(player, lineSquare, i - 3, lineMosaic[z], mosaic_square, score, i);
									}
									else
										checkSquare(player, lineSquare, i + 2, lineMosaic[z], mosaic_square, score, i);
									mosaic_square[i][i] = player->getLineSquare();
									mosaic[j][j] = mosaicClone[j][j];
									break;
								}
...
void Game::readRound(Player* player, string factory_load[6][6], string mosaic[6][6], string mosaic2[6][6], string mosaic_square[6][6], string mosaic_square2[6][6], int round, int scoreForPlayer1, int scoreForPlayer2) {
...
bool Game::check_input_turn(int factory_line, char title_code, int storage_row) {
	if (factory_line > 5 || factory_line < 0)
		return false;
	if (storage_row > 5 || storage_row < 0)
		return false;
...
bool Game::check_same_color(char title_code, int storage_row, string m[6][6]) {
...
bool Game::check_square_mosaic(int factory_line, char title_code, int storage_row, string mosaic_square[6][6]) {
	for (int i = 1; i <= 5; i++) {
...
void Game::Cal_score(int &score, string mosaic_square[6][6], int position, int lineS) {
	int point = 1;
	int counter = 0;
	int j = position;
